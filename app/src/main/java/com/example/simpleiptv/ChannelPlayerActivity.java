...
public class ChannelPlayerActivity extends AppCompatActivity implements Player.Listener {
    ...
    private static final long PLAYER_INITIALIZATION_TIMEOUT_MS = 10000; // 30 seconds
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        hdchannel = findViewById(R.id.hdchannel);
        ...
        // Find the position of currentChannel in the channel list
        int currentChannelPosition = -1;
        for (int i = 0; i < IPTVApplication.channelList.size(); i++) {
            if (IPTVApplication.channelList.get(i).equals(currentChannel)) {
                currentChannelPosition = i; // Found the channel, store its position
                break; // Exit the loop since the channel is found
            }
        }

        if (currentChannelPosition != -1) {
            // Scroll RecyclerView to the position of currentChannel
            recyclerView.scrollToPosition(currentChannelPosition);
        } else {
            // Handle the scenario where currentChannel is not found in the list
            // For example:
            // Toast.makeText(context, "Channel not found", Toast.LENGTH_SHORT).show();
        }
    }
                ...
                public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
                    if (playbackState == Player.STATE_READY || playbackState == Player.STATE_ENDED) {
                        ...
                        if (player.getVideoFormat() != null) {
                            ...
                            if (width >= 760) {
                                hdchannel.setVisibility(View.VISIBLE);
                            } else {
